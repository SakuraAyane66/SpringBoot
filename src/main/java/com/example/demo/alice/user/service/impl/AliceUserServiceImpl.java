...
import com.example.demo.common.utils.SaltUtil;
import com.example.demo.common.utils.TokenUtil;
...
//            user.setCreat_time(DateTransUtil.getDateTime(System.currentTimeMillis())); //在mybatis中自动创建时间
    ...
    @Override
    public Map<String,String> login(AliceUser user) {
...
//        }else{//失败返回}
        ...
        if(password.equals(trueUser.getPassword())){
            ...
            //此处应该和返回token的逻辑分开，需要加入到redis队列中去执行，毕竟要考虑高并发的时候
            //
